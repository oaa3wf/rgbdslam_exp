...
typedef std::set<g2o::HyperGraph::Edge*> EdgeSet;
	...
	struct InvChi2CostFunction: public g2o::HyperDijkstra::CostFunction {
 		virtual double operator ()(g2o::HyperGraph::Edge* edge, g2o::HyperGraph::Vertex* from, g2o::HyperGraph::Vertex* to);
		};

		double InvChi2CostFunction::operator () (g2o::HyperGraph::Edge* edge,
				       g2o::HyperGraph::Vertex* from __attribute__((unused)), 
				       g2o::HyperGraph::Vertex* to __attribute__((unused)) ) {  
    ...
    calibration_vertex_(NULL)
{
 ...
 next_landmark_id = 0;
  ...
  timer_ = nh.createTimer(ros::Duration(0.1), &GraphManager::broadcastLatestTransform, this);
	fp2.open("/home/sastrygrp/Documents/correspondences/time_log.txt",std::ofstream::app | std::ofstream::binary);
}
    ...
    if (strategy == "previous" && graph.size() > 2) {
      ...
      for (std::map<int, Node*>::iterator it=graph.begin(); it!=graph.end(); ++it){
        ...
        optimizer->vertex(mynode->vertex_id_)->setFixed(is_outside_largest_loop);
				//optimizer->vertex(mynode->vertex_id_)->setFixed(true);
      }
      //ROS_INFO("%s", ss.str().c_str());
    }
...
double GraphManager::optimizeGraphImpl(double break_criterion)
{
  ...
  else //Got the lock
  {
   ...
   if (ps->get<std::string>("pose_relative_to") == "inaffected") {
     ...
     g2o::HyperDijkstra hypdij(optimizer_);
     InvChi2CostFunction cost_function;
	...
	//Dapo added
        ...
        //ROS_WARN("Vertex ID: %d", vid);
		 ...
		 optimizer_->setFixed(vs2, false);

	//
     optimizer_->initializeOptimization(vs2);
    ...
    //Optimize to convergence
      ...
      do {
        ...
        chi2 = optimizer_->chi2();
			...
			t2=clock();
			std::string text;
			std::sprintf((char*)text.c_str(),"%lu,%lu,%f\n",optimizer_->activeVertices().size(),optimizer_->activeEdges().size(),((float)t2-(float)t1)/CLOCKS_PER_SEC);
